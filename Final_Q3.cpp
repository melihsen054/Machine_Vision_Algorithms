...
Mat train_images[60000];
char train_labels[60000];
char test_labels[10000];
float features [60000][9];// area, compactness, Hu moment
float testfeatures[10000][9];// area, compactness, Hu moment
void read_testdata(const char* feature_filename, const char* label_filename){
	...
	// Open files
    std::ifstream feature_file(feature_filename, std::ios::in | std::ios::binary);
	...
	char label;
	float feature;
    for (int item_id = 0; item_id < num_labels; ++item_id) {
		for (int i=0;i<9;i++){		
			feature_file.read((char*)&feature,sizeof(float));
			testfeatures[item_id][i]=feature;
		}
        // read label
...
void read_data(const char* image_filename, const char* label_filename){
    ...
    for (int item_id = 0; item_id < num_items; ++item_id) {
        ...
        label_file.read(&label, 1);
		train_labels[item_id]=label;
        Mat image_tmp(rows,cols,CV_8U,pixels);
		train_images[item_id]=image_tmp.clone();
    }
    delete[] pixels;
}
...
int main(){
	read_data("Database/MNIST/train-images.idx3-ubyte", "Database/MNIST/train-labels.idx1-ubyte");
	int mode=2;	
	if(mode==1){
		read_testdata("Database/MNIST/testfeatures-gaussnoise.idx1-ubyte", "Database/MNIST/t10k-labels.idx1-ubyte");
	}else if(mode==2){
		read_testdata("Database/MNIST/testfeatures-spnoise.idx1-ubyte", "Database/MNIST/t10k-labels.idx1-ubyte");
	}	
	
	for(int ioimg=0;ioimg<60000;ioimg++){
	image1=train_images[ioimg].clone();
	thresholdimage(100);
	Region_Labelling();
	contour_calc();
    int correctlabel=0;
...
////////////////////////////////////////////////////////////////////////////////////
	//Bayesian Classifier///////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////
	float midpoints[10][9];
	int numberofclass[10]={0};
	float sumofclass[10][9]={0};
	for(int i=0;i<60000;i++){
		numberofclass[int(train_labels[i])]++;
		for(int a=0;a<9;a++){
			sumofclass[int(train_labels[i])][a]+=features[i][a];
		}
	}
	for(int a=0;a<10;a++){
		for(int b=0;b<9;b++){
			midpoints[a][b]=sumofclass[a][b]/(float)numberofclass[a];
		}
	}
	int predictedlabelbayes[10000];
	float mindist;
	float accuracybayessian=0;
	for(int i=0;i<10000;i++){
		for(int a=0;a<10;a++){
			float dist=0;
			for(int b=0;b<9;b++){
				dist +=	(midpoints[a][b]-testfeatures[i][b])*(midpoints[a][b]-testfeatures[i][b]); 
			}
			dist = sqrt (dist);
			if(a==0){
				mindist= dist;
				predictedlabelbayes[i]=a;
			}else{
				if(dist < mindist){
					mindist= dist;
					predictedlabelbayes[i]=a;
		...
		if(predictedlabelbayes[i]==int(test_labels[i])){
			accuracybayessian++;
		}
	}
	accuracybayessian /= (float)10000;
	cout<< "Bayessian accuracy: "<<	accuracybayessian<< endl;
...
/////////////////////////////////////////////////////////////////////////////////////
	int K=10; 	
	float accuracyknn=0;
	float mindistknn[K]={0};	
	int predictedlabelknn[10000][K];
	int knnpredicted[10000];
	int a=0;
	int b=0;
	float max;
	int melih;
	...
	for(int d=0;d<K;d++){
		...
		for(int e=d;e<K;e++){
			if(e==d){
				...
				if(max<mindistknn[e]){
					max=mindistknn[e];
					float tempdist=mindistknn[d];
					mindistknn[d]=mindistknn[e];
					mindistknn[e]=tempdist;
					int templabel=predictedlabelknn[a][d];
					predictedlabelknn[a][d]=predictedlabelknn[a][e];
					predictedlabelknn[a][e]=templabel;				
				}
			}
		}
	}
	...
	for (a=0;a<10000;a++){
		for(b=0;b<60000;b++){
			dist=0;
			for(int c=0;c<9;c++){
				dist+= (features[b][c]-testfeatures[a][c])*(features[b][c]-testfeatures[a][c]);
			}
			if(b<K-1){
				mindistknn[b]=dist;
				predictedlabelknn[a][b]=int(train_labels[b]);
			}
			else{
				if(b==K-1){
						mindistknn[b]=dist;
				...
				if(dist<mindistknn[0]){
						mindistknn[0]=dist;
						predictedlabelknn[a][0]=int(train_labels[b]);
				}
				goto sortpredict;
				cont2:
				melih=0;
		...
		for(int ab=0;ab<K;ab++){
			plabels[predictedlabelknn[a][ab]]++;
		}
		int maxlabel=0;
		for(int ac=0;ac<K;ac++){
			if(plabels[ac]>maxlabel){
				maxlabel=plabels[ac];
				knnpredicted[a]=ac;			
			}
		}
		if(knnpredicted[a]==int(test_labels[a])){
			accuracyknn++;
		}
	}
	accuracyknn /= (float)10000;
	cout<< "KNN accuracy: "<<	accuracyknn<< endl;
///////////////////////////////////////////////////////////////////////////////////////
	return 0;
}
